...
//nilai
$filenya = "nilai_sertifikat.php";
$judul = "Calon : Nilai Sertifikat";
...
if ($_POST['btnSMP'])
	...
	do
		...
		//cek null
		$qcc = mysql_query("SELECT * FROM psb_siswa_calon_sertifikat ".
		...
		if ($tcc != 0)
			{
			//entry update
			mysql_query("UPDATE psb_siswa_calon_sertifikat SET nilai = '$xbnilx', ".
							"total = '$xbnilx', ".
							"postdate = '$today' ".
							"WHERE kd_siswa_calon = '$swkd' ".
							"AND kd_sertifikat = '$d_kd'");
			...
			{
			mysql_query("INSERT INTO psb_siswa_calon_sertifikat(kd, kd_siswa_calon, kd_sertifikat, nilai, total, postdate) VALUES ".
							"('$x', '$swkd', '$d_kd', '$xbnilx', '$xbnilx', '$today')");
			}
		}
...
<a href="nilai.php">Nilai-Nilai</a> > Nilai Sertifikat
<br>
<br>
...
$qc1 = mysql_query("SELECT SUM(total) AS totalku ".
						"FROM psb_siswa_calon_sertifikat ".
						"WHERE kd_siswa_calon = '$swkd'");
