...
$judul = "Data Mata Pelajaran Ujian Tertulis";
...
if ($s == "edit")
	...
	//query
	$qx = mysql_query("SELECT * FROM psb_m_mapel2 ".
...
if ($_POST['btnSMP'])
	...
	if (empty($s))
		{
		mysql_query("INSERT INTO psb_m_mapel2(kd, kd_tapel, no, mapel, bobot) VALUES ".
	...
	//jika update
		...
		{
		//update
		mysql_query("UPDATE psb_m_mapel2 SET no = '$x_no', ".
						"mapel = '$x_mapel', ".
						"bobot = '$x_bobot' ".
						"WHERE kd = '$kd'");
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysql_query("DELETE FROM psb_m_mapel2 ".
						"WHERE kd_tapel = '$tp_tapelkd' ".
						"AND kd = '$kd'");
		}
...
//query
$q = mysql_query("SELECT * FROM psb_m_mapel2 ".
					"WHERE kd_tapel = '$tp_tapelkd' ".
					"ORDER BY no ASC");
...
if ($total != 0)
	...
	echo '<p>
	<font color="red">
	<strong>Belum Ada Data Mata Pelajaran, Ujian Tertulis.</strong>
	</font>
	</p>';
	}
